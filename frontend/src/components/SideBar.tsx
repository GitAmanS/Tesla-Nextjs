    ...
    <div className="fixed flex flex-col h-screen w-[17%] bg-[#FFFFFF] text-white px-5 border border-[#EFF0F6] rounded-3xl">
          ...
          <Link href={link.path} key={link.name}>
            <li
              className={`px-4 py-3 my-1 w-full cursor-pointer rounded-lg text-[14px] font-[500] transition-all duration-300 ease-in-out ${
                pathname === link.path.toLowerCase() ? 'bg-[#1B59F81A] text-[#1B59F8]' : 'text-[#000000B2]'
              }`}
            >
              {link.name}
            </li>
      ...
      <ul className="pb-2">
        {support.map((link) => (
          <Link href={link.path} key={link.name}>
            <li
              className={`px-4 py-3 my-1 w-full cursor-pointer rounded-lg text-[14px] font-[500] transition-all duration-300 ease-in ${
                pathname === link.path.toLowerCase() ? 'bg-[#1B59F81A] text-[#1B59F8]' : 'text-[#000000B2]'
              }`}
            >
              {link.name}
      ...
      <div className="flex flex-col px-4 py-8 items-start justify-start border-t mt-auto">
        ...
        />
        <h1 className="text-[#000000] font-[600] text-[14px]">Sam Wheeler</h1>
        <p className="text-[#00000080] font-[400] text-[12px]">samwheeler@example.com</p>
      </div>
    </div>
  );
};
